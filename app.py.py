...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///dpobcms.db'
...
# Ensure upload directory safely exists
...
class OBEntry(db.Model):
    ...
    status = db.Column(db.String(50), default='Pending Review') # Pending Review, Under Investigation, Arrest Made, Court Process, Closed
    station_id = db.Column(db.Integer, db.ForeignKey('stations.id'), nullable=False)
...
class InvestigationNote(db.Model):
    ...
    note_type = db.Column(db.String(50), nullable=False) # Timeline Update, Witness Statement, Arrest Record, General Note
...
@login_required
def dashboard():
    # Role-Based Access Cross-Station Isolation
...
def ob_management():
    if request.method == 'POST':
        # Autogenerated Serialized Key Sequence Logic
        ...
        # Instantiate base investigative mapping layer
